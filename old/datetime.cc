...
namespace {
  ...
  inline bool validate_result(struct std::tm * result, const int year)
  {
    assert(result);

    if (result->tm_year == 0)
      result->tm_year = (year == -1 ? date_t::current_year : year) - 1900;
		       ...
		       const int year = -1)
  {
    if (! date_t::input_format.empty()) {
      std::memset(result, 0, sizeof(struct std::tm));
#ifdef HAVE_STRPTIME
      if (strptime(date_str, date_t::input_format.c_str(), result) &&
	  validate_result(result, year))
	return true;
#endif
    }
		  ...
		  const int year = -1)
  {
    ...
    if (! parse_date_mask(date_str, &when, year))
      return false;
